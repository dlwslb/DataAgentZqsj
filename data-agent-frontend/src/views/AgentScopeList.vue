<!--
 * AgentScope 智能体列表页
 * 基于 AgentList.vue 改造，对接 data-agent-scope 后端
-->
      ...
      <main class="main-content">
        ...
        <div class="content-header">
          <div class="header-info">
            <h1 class="content-title">AgentScope 智能体</h1>
            <p class="content-subtitle">基于 AgentScope 框架的智能体管理</p>
          </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const agents = ref<AgentScope[]>([]);

      // 计算属性
      const publishedCount = computed(
        () => agents.value.filter((a: AgentScope) => a.status === 'published').length,
      );
      const draftCount = computed(
        () => agents.value.filter((a: AgentScope) => a.status === 'draft').length,
      );
      const offlineCount = computed(
        () => agents.value.filter((a: AgentScope) => a.status === 'offline').length,
      );
      ...
      const filteredAgents = computed(() => {
        ...
        if (activeFilter.value !== 'all') {
          filtered = filtered.filter((agent: AgentScope) => agent.status === activeFilter.value);
        }
      ...
      const loadAgents = async () => {
        ...
        try {
          ...
          agents.value = response.data?.data || response.data || response || [];
        } catch (error) {
      ...
      // 删除智能体
      const handleDeleteAgent = async (agent: AgentScope) => {
        try {
          ...
          await agentScopeApi.delete(agent.id);
          ElMessage.success('智能体删除成功');
          // 从列表中移除已删除的智能体
          agents.value = agents.value.filter((a: AgentScope) => a.id !== agent.id);
        } catch (error) {
          if (error !== 'cancel') {
