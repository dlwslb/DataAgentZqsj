        ...
        HarnessAgent.Builder builder = HarnessAgent.builder()
                ...
                .sysPrompt("""
                            ...
                            * **datePreset**: 指令含"今日"或"昨日"时设为 `today`/`yesterday`。
                            ...
                            ### 3. 结果输出
                            * **⭐ 金额单位统一为万元**：所有金额字段（win_bid_price / bidding_budget / win_bid_amount / budget_amount）Tool 已自动从元转万元，保留 2 位小数。响应 JSON 里有 `unit: "万元"` 标识。表格列必须写"金额（万元）"，对应数值直接展示不要再加"元"或"万元"后缀。
                            * **⭐ 标题以 SKILL 输出模板为准**（不要再自己造标题）:
                              - **首选**：按对应 SKILL 的"## 输出模板"段的标题来（如 `## 中标信息查询结果` / `## 招标信息查询结果` / `## 今日/昨日标讯（共 N 条）`），**禁止**自己改成 `# X 报告` 或其他格式
                              - 标题层级用 `##` 二级（不是 `#` 一级）
                              - 标题里的业务类型用词参考下表（与 SKILL 标题保持一致,不要 LLM 自己造）:
                                | bizType | 业务类型用词 |
                                |---------|------------|
                                | bid_winner | 中标信息 |
                                | bidding | 招标信息 |
                                | purchase_intention | 采购意向 |
                                | prepose | 拟在建项目（部分 SKILL 标"前期项目"）|
                                | origin_announcement | 标讯（配合时间: 今日/昨日标讯）|
                              - **兜底**（SKILL 没明确标题时）: 用 `## {业务类型}查询结果`（二级标题,不要用 `#` 一级）
                              - 详情标题按 detail-* SKILL 的"## 输出模板"段来（如 `## XX 项目招标详情`）
                            * **数据原样输出铁律**：
