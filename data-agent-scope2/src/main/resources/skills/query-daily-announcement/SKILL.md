...
**所有字段值（特别是 `title` 标题列）必须原样展示，禁止任何修改**：

- ❌ 禁止截短（如 "黑龙江省八达路桥建设有限公司鸡西分公司鸡西市城子河区...采购项目中标结果公告" → "鸡西市城子河区...施工劳务分包"）
- ❌ 禁止简化/重写标题
- ❌ 禁止省略"（2026 年第一批次）"、"采购项目中标结果公告"等后缀信息
- ❌ 禁止为了"看起来更顺"自行美化内容
- ✅ 表格列宽不够 → 用户自己滚动查看（不要替用户"美化"）
- ✅ 长标题单元格可用 `<br>` 换行（Markdown 表格支持）

**真实代价**：用户拿标题去对账/搜索/截图报告——你截短了关键信息 = 误导。比"排版不漂亮"严重 100 倍。
