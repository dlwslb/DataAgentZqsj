...
|--------------|-----------------|------------|
| 中标、中标信息、中标公告、中标结果、中标候选人公示、合同公告 | `中标` | `LIKE '%中标%' OR LIKE '%合同公告%'` |
| 招标、招标公告、招标预告、招标文件 | `招标` | `LIKE '%招标%'` |
| 采购、采购意向、政府采购 | `采购` | `LIKE '%采购%'` |
| 审批项目、拟在建、前置商机、前期项目 | `前置商机` | `LIKE '%审批项目%' OR LIKE '%拟在建%' OR LIKE '%前期项目%' OR LIKE '%前置商机%'` |
| **没有明确类型**（纯"今日/昨天"无任何分类词） | **不传 channel**（查全量） | - |

> ⭐ **强制规则**（避免 LLM 漏传 channel）：
> 1. **用户问题只要含以上类型关键词中的任意一个**，**必须**在 conditions 里传 `channel` 字段
> 2. **典型误判案例**："黑龙江今日中标" — 这里的"中标"是**分类词**，不是修饰语，**必须**传 `channel="中标"`
