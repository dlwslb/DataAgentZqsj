---
name: detail-bidding
description: 【招标信息详情 - 强优先】当用户问**某个具体招标项目/项目标题/项目编号**的招标信息时使用本 skill。**触发特征**:用户问题里**必须含具体项目名/项目编号**(典型问法:"XX 项目的招标""查一下 XX 招标的联系方式""XX 招标的投标截止时间""XX 招标公告的内容")。调 `get_bidding_detail` 工具按 `keyword` 查 1 条全字段。**关键区分**:用户问"招标有哪些/最近有什么招标/招标汇总/招标统计"用 `query-bidding`(列表);**用户问"某个具体项目的招标"用本 skill**。表 `chatbi.bid_biz_bidding`。
---
