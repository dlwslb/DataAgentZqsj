...
**触发条件**：当主表（`chatbi.bid_biz_purchase_intention`）没数据时，工具会自动用相同 keyword 去 `origin_announcement` 表（原始每日标讯）查，`channel=采购`。

**响应里怎么识别兜底**：
- `source: "origin_announcement_fallback"` ← 看到这字段就是兜底结果
- `primaryTable: "chatbi.bid_biz_purchase_intention"` ← 主表名
